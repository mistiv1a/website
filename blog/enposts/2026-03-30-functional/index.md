Adding Type Annotations to theEuler-Lagrange Equation
========

In English, the word _functional_ is generally regarded as an adjective, as in *functional programming*. It can, however, also serve as a noun, denoting a higher-order function — a function that accepts another function as its input, such as the famous `map` and `reduce`. Here is a simple JavaScript example:
...
In mathematics, however, the term _functional_ denotes something different: a mapping from functions to numbers. The calculus of variations, which relies on this concept, is the foundation of analytical mechanics in physics. *Structure and Interpretation of Classical Mechanics* is an attempt to bring the two together: it is the companion volume to _SICP_ and teaches analytical mechanics in Scheme. I first attempted to read this book two years ago, but after considerable effort the mathematical formulas still defeated me, I kept losing my place among the Lisp parentheses, and the English prose was laborious to read; in the end I abandoned it.
...
Let us open the [Wikipedia page](https://en.wikipedia.org/wiki/Euler%E2%80%93Lagrange_equation) for the Euler–Lagrange equation.

It defines something called the action functional. Its input is a trajectory function $q(t)$ that evolves over time, and it computes the integral of the Lagrangian over the time interval $[t_\text{start}, t_\text{end}]$:

$$ S[q] = \int_{t_\text{start}}^{t_\text{end}} L(t, q(t), \dot{q}(t))\, \mathrm{d}t $$
...
Look closely at the lambda expression inside: this is precisely the most confusing part of the mathematical formula. It appears that we are integrating the Lagrangian function itself, but that is not what is happening at all. In reality, the formula uses the three-variable function $L$, the trajectory function $q$, and the derivative operator $\mathrm{D}$ to define a new one-variable function, and then integrates this newly defined function. In code, this ambiguous spot in the formula has nowhere to hide, so it no longer causes confusion.
...
Here `px`, `py`, and `pz` correspond respectively to the partial derivative operators $\frac{\partial}{\partial x}, \frac{\partial}{\partial y}, \frac{\partial}{\partial z}$ acting on the function $f(x, y, z)$. Their type signature is:
...
$$ \frac{\partial L}{\partial q}(t, q(t), \dot{q}(t)) = \frac{\mathrm{d}}{\mathrm{d}t}\left( \frac{\partial L}{\partial \dot{q}}(t, q(t), \dot{q}(t)) \right) $$
...
Observe that `lhs` and `rhs` have the same type: both are functions of one variable. The meaning of the Euler–Lagrange equation is that, if the action functional $S$ is at a stationary point, then for every $t$ in the interval from $t_\text{start}$ to $t_\text{end}$ we have:
...
According to the Euler–Lagrange equation, if the action functional $S[q] = \int_{t_\text{start}}^{t_\text{end}} L(t, q(t), \dot{q}(t))\, \mathrm{d}t$ is at a stationary point, then necessarily `lhs t == rhs t`. Substituting into the formulas of the previous section:
...
$$ a(t) = \frac{\mathrm{d}^2 q}{\mathrm{d}t^2}(t) $$
...
$$ F(t) = -\frac{\mathrm{d}V}{\mathrm{d}q}(q(t)) $$
