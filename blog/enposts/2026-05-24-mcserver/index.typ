...
The main drive for building this server was that I had watched several videos on Bilibili about the _Create: Aeronautics_, so I want to try it. This mod currently supports version 1.21.1 primarily, which is also a version with good mod compatibility at present, so 1.21.1 was the natural choice.

= Client Launcher Selection

The most popular launcher at present is #link("https://prismlauncher.org/", "Prism Launcher"). However, this launcher supports only legitimate copies of the game. If you also want friends who have not purchased the game to join with a pirate copy, you need another launcher, such as #link("https://polymc.org/", "PolyMC"). The two launchers share a common origin, both being forks of #link("https://multimc.org/", "MultiMC"); but their modpacks are not interchangeable and must be packed separately.
...
Adding mods on the client side is very simple. But on the server side it's a little tricky, the mod API chosen is NeoForge, so first download the installer from the #link("https://neoforged.net/", "NeoForge website"), run it, and choose to install the server. Then open the client instance directory, locate the `mods/` directory inside it, and copy all the jar files into the mods directory of NeoForge.
