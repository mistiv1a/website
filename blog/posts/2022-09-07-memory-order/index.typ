...
不过，在x86/x64这样的架构当中，其实对所有内存中的变量的操作自动就是acquire-release的，这被称为强内存模型；相反，ARM架构下就没有这种保证了，这类架构被称为弱内存模型。可是，即使在x86/x64中，也不可以掉以轻心，编译器可能会对程序进行一些激进的优化，Load/Store操作可能会有重排。而 acquire和release则会告诉编译器，在这里重排是不允许的。例如，acquire因为需要保证该操作之前的所有读写都会被同步到主内存，所以acquire之后的读写操作不可以被重排到acquire之前；与之类似，release操作之前的读写操作也不可以被重排到release之后。
